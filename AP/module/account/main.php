...
return new class(){ //create main class
	...
	public $autoGetUserData = true; //auto get user data if is loggin
	public function __construct(){ //main function
		...
		if($this->autoGetUserData) //if auto get user data
			if($this->checkUser()) //if user is login
				$this->userGetData(); //get user data
	}
	...
	public function changePassword(string $login, string $password, string $newPassword){ //change password
		...
		core::$library->database->conn->query('UPDATE user SET password="'.$newPassword.'" WHERE id='.$user['id'])->fetch(PDO::FETCH_ASSOC); //get user from database
		return true;
	}
